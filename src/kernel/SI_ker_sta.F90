...
!-------------------------------------------------------------------------------
    ...
    integer :: num_freq,num_pick,num_stiff
    ...
    real(SP),dimension(:,:,:),allocatable :: Kap,Kaq,Kbp,Kbq,Kconv
...
!-- read all the pick's freq, twin info on this component
    ...
    !-- create or open kernel nc file
        if (flag_fst .and. (blki==1 .and. blkj==1 .and. blkk==1)) then
            ...
            if (flag_VpVs) then
                call nfseis_grid3d_defvar(ncid(npk),VNAME_KER_PHA_VP,kapid(npk))
                call nfseis_grid3d_defvar(ncid(npk),VNAME_KER_AMP_VP,kaqid(npk))
                call nfseis_grid3d_defvar(ncid(npk),VNAME_KER_PHA_VS,kbpid(npk))
                call nfseis_grid3d_defvar(ncid(npk),VNAME_KER_AMP_VS,kbqid(npk))
            end if
            if (flag_lammu) then
                call nfseis_grid3d_defvar(ncid(npk),VNAME_KER_PHA_LA_LM,LM_L_pid(npk))
                call nfseis_grid3d_defvar(ncid(npk),VNAME_KER_AMP_LA_LM,LM_L_qid(npk))
                call nfseis_grid3d_defvar(ncid(npk),VNAME_KER_PHA_MU_LM,LM_M_pid(npk))
                call nfseis_grid3d_defvar(ncid(npk),VNAME_KER_AMP_MU_LM,LM_M_qid(npk))
            end if
            if (flag_kappamu) then
                call nfseis_grid3d_defvar(ncid(npk),VNAME_KER_PHA_KA_KM,KM_K_pid(npk))
                call nfseis_grid3d_defvar(ncid(npk),VNAME_KER_AMP_KA_KM,KM_K_qid(npk))
                call nfseis_grid3d_defvar(ncid(npk),VNAME_KER_PHA_MU_KM,KM_M_pid(npk))
                call nfseis_grid3d_defvar(ncid(npk),VNAME_KER_AMP_MU_KM,KM_M_qid(npk))
            end if
            ...
            if (flag_VpVs) then
                call nfseis_inq_varid(ncid(npk),VNAME_KER_PHA_VP,kapid(npk))
                call nfseis_inq_varid(ncid(npk),VNAME_KER_AMP_VP,kaqid(npk))
                call nfseis_inq_varid(ncid(npk),VNAME_KER_PHA_VS,kbpid(npk))
                call nfseis_inq_varid(ncid(npk),VNAME_KER_AMP_VS,kbqid(npk))
            end if
            if (flag_lammu) then
                call nfseis_inq_varid(ncid(npk),VNAME_KER_PHA_LA_LM,LM_L_pid(npk))
                call nfseis_inq_varid(ncid(npk),VNAME_KER_AMP_LA_LM,LM_L_qid(npk))
                call nfseis_inq_varid(ncid(npk),VNAME_KER_PHA_MU_LM,LM_M_pid(npk))
                call nfseis_inq_varid(ncid(npk),VNAME_KER_AMP_MU_LM,LM_M_qid(npk))
            end if
            if (flag_kappamu) then
                call nfseis_inq_varid(ncid(npk),VNAME_KER_PHA_KA_KM,KM_K_pid(npk))
                call nfseis_inq_varid(ncid(npk),VNAME_KER_AMP_KA_KM,KM_K_qid(npk))
                call nfseis_inq_varid(ncid(npk),VNAME_KER_PHA_MU_KM,KM_M_pid(npk))
                call nfseis_inq_varid(ncid(npk),VNAME_KER_AMP_MU_KM,KM_M_qid(npk))
            end if
        end if
...
!-- calculate kernels for each pick on this comment
    ...
    !-- use background and scattering waves to calculate kernels with respect to lambda and mu
        ...
        call scatter2kernel(VV,UU,Uda,Udb,nt1,nt2,bsubc,Kap,Kaq,Kbp,Kbq)
    
    !-- save the kernels to nc file
        if (flag_lammu) then
            Kconv=Kap*C13 !- to relative perturbation
            call nfseis_put(ncid(npk),LM_L_pid(npk),Kconv,bsubs,bsubc,bsubt)
            Kconv=Kaq*C13
            call nfseis_put(ncid(npk),LM_L_qid(npk),Kconv,bsubs,bsubc,bsubt)
            Kconv=Kbp*C66 
            call nfseis_put(ncid(npk),LM_M_pid(npk),Kconv,bsubs,bsubc,bsubt)
            Kconv=Kbq*C66 
            call nfseis_put(ncid(npk),LM_M_qid(npk),Kconv,bsubs,bsubc,bsubt)
        end if
        if (flag_VpVs) then
            Kconv=Kap*(C13+2.0*C66)*2.0
            call nfseis_put(ncid(npk),kapid(npk),Kconv,bsubs,bsubc,bsubt)
            Kconv=Kaq*(C13+2.0*C66)*2.0
            call nfseis_put(ncid(npk),kaqid(npk),Kconv,bsubs,bsubc,bsubt)
            Kconv=2.0*C66*(Kbp-2.0*Kap)
            call nfseis_put(ncid(npk),kbpid(npk),Kconv,bsubs,bsubc,bsubt)
            Kconv=2.0*C66*(Kbq-2.0*Kaq)
            call nfseis_put(ncid(npk),kbqid(npk),Kconv,bsubs,bsubc,bsubt)
        end if
        if (flag_kappamu) then
            Kconv=Kap*(C13+C66*2.0/3.0)
            call nfseis_put(ncid(npk),KM_K_pid(npk),Kconv,bsubs,bsubc,bsubt)
            Kconv=Kaq*(C13+C66*2.0/3.0)
            call nfseis_put(ncid(npk),KM_K_qid(npk),Kconv,bsubs,bsubc,bsubt)
            Kconv=(Kbp-Kap*2.0/3.0)*C66
            call nfseis_put(ncid(npk),KM_M_pid(npk),Kconv,bsubs,bsubc,bsubt)
            Kconv=(Kbq-Kaq*2.0/3.0)*C66
            call nfseis_put(ncid(npk),KM_M_qid(npk),Kconv,bsubs,bsubc,bsubt)
        end if
...
!-------------------------------------------------------------------------------
    ...
    if (flag_lammu) then
        allocate(LM_L_pid(npk)); LM_L_pid=0
        allocate(LM_L_qid(npk)); LM_L_qid=0
        allocate(LM_M_pid(npk)); LM_M_pid=0
        allocate(LM_M_qid(npk)); LM_M_qid=0
    end if
    if (flag_kappamu) then
        allocate(KM_K_pid(npk)); KM_K_pid=0
        allocate(KM_K_qid(npk)); KM_K_qid=0
        allocate(KM_M_pid(npk)); KM_M_pid=0
        allocate(KM_M_qid(npk)); KM_M_qid=0
    end if
!-------------------------------------------------------------------------------
...
!--   thus 11 means all two parameterizations, 10 means to Vph, etc.
    ...
    elseif (n==2) then
        call error_except("use SI_ker_sta_aniso for VTI medium")
        flag_VTI=.true.
        num_stiff=5
        n=len_trim(str)
    ...
    elseif (n==3) then
        call error_except("use SI_ker_sta_aniso for anisotropic medium")
        flag_GENE=.true.
        num_stiff=21
...
!-- For VTI anisotropic media
    if (flag_VTI) then
        ...
        E1=(ts11*Exx(i,j,k,n)+ts12*Eyy(i,j,k,n)+ts13*Ezz(i,j,k,n))/det_A
        E2=(ts12*Exx(i,j,k,n)+ts11*Eyy(i,j,k,n)+ts13*Ezz(i,j,k,n))/det_A
        E3=(ts13*Exx(i,j,k,n)+ts13*Eyy(i,j,k,n)+ts33*Ezz(i,j,k,n))/det_A
        Exx(i,j,k,n)=E1
        Eyy(i,j,k,n)=E2
        Ezz(i,j,k,n)=E3
        Exy(i,j,k,n)=ts44*Exy(i,j,k,n)
        Exz(i,j,k,n)=ts44*Exz(i,j,k,n)
        Eyz(i,j,k,n)=ts44*Eyz(i,j,k,n)
    end if
...
!-- For isotropic media
    if (flag_ISO) then
        ...
        Exy(i,j,k,n)=E3*Exy(i,j,k,n)
        Exz(i,j,k,n)=E3*Exz(i,j,k,n)
        Eyz(i,j,k,n)=E3*Eyz(i,j,k,n)
    end if
...
!-- normalization
    ...
    !-- Forming kernels for tau_p and tau_q.
        Kap(i,j,k)=0.5*(V(nt1)*Ka(nt1)+V(nt2)*Ka(nt2))
        Kaq(i,j,k)=0.5*(U(nt1)*Ka(nt1)+U(nt2)*Ka(nt2))
        Kbp(i,j,k)=0.5*(V(nt1)*Kb(nt1)+V(nt2)*Kb(nt2))
        Kbq(i,j,k)=0.5*(U(nt1)*Kb(nt1)+U(nt2)*Kb(nt2))
        do n=nt1+1,nt2-1
            Kap(i,j,k)=Kap(i,j,k)+V(n)*Ka(n)
            Kaq(i,j,k)=Kaq(i,j,k)+U(n)*Ka(n)
            Kbp(i,j,k)=Kbp(i,j,k)+V(n)*Kb(n)
            Kbq(i,j,k)=Kbq(i,j,k)+U(n)*Kb(n)
        end do
