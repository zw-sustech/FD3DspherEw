...
!-------------------------------------------------------------------------------
!
! $Date: 2010-03-01 20:19:26 -0500 (Mon, 01 Mar 2010) $
! $Revision: 90 $
...
! Dependent modules:
!-------------------------------------------------------------------------------
    use mpi_mod, only : thisid,absnode,freenode
...
!-------------------------------------------------------------------------------
    ...
    real(SP),dimension(SEIS_GEO,2) :: Vp,Vs,fc,bmax,Rpp,dmax,amax
    real(SP) :: x0,y0,z0,L0,Lx,Ly,Lz,dh
...
!-- open configure file and read the parameters --
...
!-- PML layer toward -x direction
    if (abs_number(1,1)>0) then
       isx1=.true.
       x0=x(abs_number(1,1)+ni1)   !-- PML-interior location
       ...
       dmax(1,1)=cal_pml_dmax(L0,Vp(1,1),Rpp(1,1),bmax(1,1))
...
!-- PML layer toward +x direction
    if (abs_number(1,2)>0) then
       ...
       dmax(1,2)=cal_pml_dmax(L0,Vp(1,2),Rpp(1,2),bmax(1,2))
...
!-- PML layer toward -y direction
    if (abs_number(2,1)>0) then
       ...
       dmax(2,1)=cal_pml_dmax(L0,Vp(2,1),Rpp(2,1),bmax(2,1))
...
!-- PML layer toward +y direction
    if (abs_number(2,2)>0) then
       ...
       dmax(2,2)=cal_pml_dmax(L0,Vp(2,2),Rpp(2,2),bmax(2,2))
...
!-- PML layer toward -z direction
    if (abs_number(3,1)>0) then
       ...
       dmax(3,1)=cal_pml_dmax(L0,Vp(3,1),Rpp(3,1),bmax(3,1))
...
!-- PML layer toward +z direction
    if (abs_number(3,2)>0) then
       ...
       dmax(3,2)=cal_pml_dmax(L0,Vp(3,2),Rpp(3,2),bmax(3,2))
...
[dnl
!===============================================================================
subroutine Lx[$1]_Ly[$2]_Lz[$3]
!===============================================================================
!-- code generated by [$0($1) in] __file__ --
!-- number of argument: $#
    integer :: n,i,j,k
...
!===============================================================================
!-- use m4 macro to generate bias difference operators
!===============================================================================
...
!===============================================================================
!-- the Tiz is imaged in main code, here other components are imaged in a
!-- general anisotroic medium
#ifdef AnisGene
    integer i,j,m
    if (freenode) then
    if (isx1) then
...
!===============================================================================
!-- subroutines to calculate parameter profiles
!===============================================================================
...
!-------------------------------------------------------------------------------
function cal_pml_bmax(Vs,fc,dh) result(bmax)
!-------------------------------------------------------------------------------
    real(SP) :: Vs,fc,dh
    real(SP) :: bmax
    bmax=Vs/(0.5*6*fc*dh)
!-------------------------------------------------------------------------------
end function cal_pml_bmax
!-------------------------------------------------------------------------------
