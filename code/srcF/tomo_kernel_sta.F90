...
integer,dimension(SEIS_GEO) :: blksiz,blknum,blkfst
...
do blki=blkfst(1),blknum(1)
  ...
  write(*,*) idstr, ': current time ',str_d1, &
   ...
   bsubs=(/ (blki-1)*blksiz(1)+1,(blkj-1)*blksiz(2)+1,(blkk-1)*blksiz(3)+1 /)
...
do npk=1,picknum(ncmp)
   ...
   ! kernel nc file
   if (blki==1 .and. blkj==1 .and. blkk==1) then
...
do n=1,SEIS_GEO
 ...
 call string_conf(fid,1,'block_starter',n+1,blkfst(n))
end do
...
integer,intent(in) :: NOD,NFACT,NEL
